...
class Router:
    ...
    def stats(self, user_text, user_id):
        categories = get_unique_categories(self.db_name, user_id)
        stats = self.chatgpt.stats(user_text, categories)

        return get_aggregated_expenses(self.db_name, user_id, stats['start_date'], stats['end_date'], stats['categories'])
